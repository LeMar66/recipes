...
function fprocessUIJson(jsonDoc) {
  ...
  if (jsonDoc.type === "accordion") {
    // Maak de hoofdcontainer van de accordeon
    const acc = document.createElement("div");
    acc.className = jsonDoc.type;
    acc.id = jsonDoc.id;

    // CRUCIAAL: Sla de data op zodat we later diepere lagen kunnen vinden
    acc_data = jsonDoc;

    if (jsonDoc.translate === false) acc.setAttribute("translate", "no");
    uicontent.appendChild(acc);

    // Teken ALLEEN de eerste laag (bijv. Chinese, Indonesian, Greek)
    if (jsonDoc.items) {
      jsonDoc.items.forEach((jsonItem) => {
        // fcreateACCItem maakt de header en de lege body-container
        fcreateACCItem(acc, jsonItem);
      });
    }
...
function fcreateACCItem(addToBodyContent, json_item) {
  ...
  acc_item_header.onclick = function () {
